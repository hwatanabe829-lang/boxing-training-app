...
// ===== 音声コントロール =====
const VOICE_PHASE_MAP = [
  { keywords: ["ウォームアップ", "ウォーム", "準備運動"],        phase: "warmup" },
  { keywords: ["シャドー", "シャドウ", "シャドボ"],              phase: "shadow" },
  { keywords: ["ミット"],                                         phase: "mitt" },
  { keywords: ["サンドバック", "サンドバッグ", "バッグ"],         phase: "bag" },
  { keywords: ["フットワーク"],                                   phase: "footwork" },
  { keywords: ["目慣れ", "アイボクシング", "目なれ"],             phase: "eyeboxing" },
  { keywords: ["受け返し", "打ち返し", "返し"],                   phase: "counterreturn" },
  { keywords: ["マス", "マスボクシング"],                         phase: "massboxing" },
  { keywords: ["スパー", "スパーリング"],                         phase: "sparring" },
  { keywords: ["ラッシュ", "ラッシュバッグ"],                     phase: "rushbag" },
  { keywords: ["コンディショニング", "体幹", "腹筋", "筋トレ"],   phase: "core" },
  { keywords: ["クールダウン", "クール", "ストレッチ"],           phase: "cooldown" },
];
...
function jumpToPhase(phase) {
  if (!currentMenu) {
    announce("メニューを生成してください");
    setVoiceStatus("メニューがありません");
    return;
  }
  const round = currentMenu.rounds.find(r => r.phase === phase);
  if (!round) {
    announce("このメニューにその練習はありません");
    setVoiceStatus("見つかりませんでした");
    return;
  }
  const row = document.getElementById(rowIdFor(round));
  if (row) {
    row.scrollIntoView({ behavior: "smooth", block: "center" });
    document.querySelectorAll(".menu-table tbody tr").forEach(tr => tr.classList.remove("voice-highlight"));
    row.classList.add("voice-highlight");
    setTimeout(() => row.classList.remove("voice-highlight"), 3000);
  }
  announce(`${round.phaseName}です`);
  setVoiceStatus(`→ ${round.phaseName}`);
}

function handleVoiceCommand(text) {
  // タイマー操作
  if (/スタート|開始/.test(text)) { startTimer(); setVoiceStatus("開始"); return; }
  if (/ストップ|停止|止め|一時停止/.test(text)) { if (timerInterval) startTimer(); setVoiceStatus("一時停止"); return; }
  if (/リセット/.test(text)) { if (currentMenu) resetTimerState(); setVoiceStatus("リセット"); return; }

  // フェーズ移動
  for (const { keywords, phase } of VOICE_PHASE_MAP) {
    if (keywords.some(k => text.includes(k))) {
      jumpToPhase(phase);
      return;
    }
  }
  announce("もう一度言ってください");
  setVoiceStatus("聞き取れませんでした");
}
...
(function setupVoice() {
  ...
  if (!SR) {
    btn.title = "このブラウザは音声認識に対応していません";
  ...
  function createRec() {
    ...
    r.onresult = (e) => {
      ...
      if (!awaitingCommand) {
        // ウェイクワード検出
        if (text.includes("ボックス") || text.includes("ボクシング")) {
          beep(660, 0.1);
          // 同じ発話内にコマンドも含まれていればすぐ処理
          const rest = text.replace(/ボックス|ボクシング/, "").trim();
          if (rest.length > 0 && VOICE_PHASE_MAP.some(m => m.keywords.some(k => rest.includes(k)))) {
            handleVoiceCommand(rest);
          } else {
            awaitingCommand = true;
            setVoiceStatus("どこに移動しますか？", true);
            announce("はい");
            setTimeout(() => { rec = createRec(); rec.start(); }, 800);
          }
        } else {
          setVoiceStatus("「ボックス」と言って起動");
        }
      } else {
    ...
    r.onerror = () => {
      awaitingCommand = false;
      setVoiceStatus("エラー。もう一度押してください");
  ...
  btn.addEventListener("click", () => {
    ...
    awaitingCommand = false;
    rec = createRec();
    setVoiceStatus("「ボックス」と言って起動", true);
    try { rec.start(); } catch(e) { setVoiceStatus("マイクを許可してください"); }
  });
})();
